...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    Compatible with timm >= 0.9.0
    """
    def __init__(self, global_pool=False, **kwargs):
        # For timm >= 0.9.0, pass empty string to disable parent's pooling
        # We'll handle pooling ourselves in forward_features
        super(VisionTransformer, self).__init__(global_pool='', **kwargs)

        # Store our own global_pool flag (can be True/False)
        self.use_global_pool = global_pool
        if self.use_global_pool:
            norm_layer = kwargs['norm_layer']
            embed_dim = kwargs['embed_dim']
            self.fc_norm = norm_layer(embed_dim)
            # Don't delete self.norm - parent may not have it in newer timm

    def forward_features(self, x, **kwargs):
        # Note: **kwargs added for timm>=0.9.0 compatibility (accepts attn_mask, etc.)
        B = x.shape[0]
    ...
    def forward(self, x):
        # Override forward completely to bypass timm's forward_head pooling
